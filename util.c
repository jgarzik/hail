...
{
	timer->fired = false;
	g_queue_insert_sorted(cld_srv.timers, timer, timer_cmp, NULL);
}

int timer_next(void)
{
	struct timer *timer = g_queue_peek_head(cld_srv.timers);
	if (!timer)
		return -1;
	...
	return (timer->timeout - current_time) * 1000;
}
...
void timers_run(void)
{
	while (1) {
		...
		timer = g_queue_peek_head(cld_srv.timers);
		if (current_time < timer->timeout)
			break;

		g_queue_pop_head(cld_srv.timers);
